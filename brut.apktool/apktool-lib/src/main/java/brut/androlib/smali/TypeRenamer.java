...
public class TypeRenamer {
    ...
    public static String renameType(String type) {
        ...
        // Preserve Android resource R class verbatim, including its inner classes
        // (R$id, R$xml, etc.). R is auto-generated by AAPT and frequently looked up
        // by name (Class.forName, R$styleable reflection in AndroidX/Google libs).
        // Renaming R or its short inner names breaks resource resolution.
        if (renameClassNames && classParts.length > 0 && classParts[0].equals("R")) {
            result.append(classAndInner);
            result.append(";");
    ...
    public static String unrenameType(String type) {
        ...
        // Symmetric to renameType: leave Android R class untouched. Without this
        // guard the heuristic in unrenameOuterClass would lowercase "R" → "r"
        // because a single uppercase letter looks like a renamed default-package
        // class to the inverter.
        if (classParts.length > 0 && classParts[0].equals("R")) {
            result.append(classAndInner);
            result.append(";");
            return result.toString();
        }
